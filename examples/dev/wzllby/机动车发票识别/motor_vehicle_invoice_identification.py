...
class MotorVehicleInvoiceIdentificaion():
            ...
            if "机动车发票" in dirs:
                ...
                for filename in os.listdir(motor_dir):
                    ...
                    if os.path.isfile(file_path) and "新车发票" in filename:
                        ...
                        # 复制文件并保留元数据
                        try:
                            print(f"正在移动第{counter}张发票!")
                            shutil.copy2(file_path, new_filepath)
                            counter += 1
                        except Exception as e:
                            print(f"复制文件 {file_path} 失败: {str(e)}")

    def main_exec(self, origin_zip_file):
        ...
        try:
            ...
            if secrets is None or len(secrets) != 2:
                ...
                return
            # 获取所有的发票并且移动到新的文件中
            self.copy_rename_invoices(process_file_path)
        finally:
    ...
    def main_exec_single(self, origin_zip_file, result_excel_file):
        ...
        if secrets is None or len(secrets) != 2:
            ...
            return
        result = []
        if os.path.isfile(origin_zip_file):
            # 解析文件获取信息 第二个和第三个参数可直接在网站复制
            result_info = self.get_img_info(origin_zip_file, secrets[0], secrets[1])
            result.append(result_info)
        else:
            for item in os.listdir(origin_zip_file):
                print(f"识别图片{item}成功!")
                item_path = os.path.join(origin_zip_file, item)
                result_info = self.get_img_info(item_path, secrets[0], secrets[1])
                result.append(result_info)
...
if __name__ == '__main__':
    ...
    # zip文件的路径
    mon.main_exec("file/样例.zip")
    mon.main_exec_single("result/new_dir", "./result/output.xlsx")
